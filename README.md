...
| Nearby discovery | Feed, search, veg/non-veg filter, trend cards, and nearby merchant map. | Relevant offers instead of a global, stale catalogue. | MongoDB GeoJSON / `2dsphere` search returns merchants within 10 km, then active unexpired listings. |
...
```mermaid
flowchart LR
  C[Consumer browser\nReact SPA] -->|HTTPS / Axios| API[Express API\nNode.js]
  M[Merchant browser\nReact SPA] -->|HTTPS / Axios| API
  C <-->|Authenticated Socket.IO| RT[Socket.IO server]
  M <-->|Authenticated Socket.IO| RT
  API --> DB[(MongoDB Atlas\nUsers · Listings · Claims)]
  API --> CL[Cloudinary\nimage storage + CDN]
  API --> ORS[OpenRouteService\nwalking directions]
  C --> OSM[OpenStreetMap tiles\nvia Leaflet]
...
```mermaid
flowchart TD
  A[Listing created/updated or reminder due] --> B{Consumer within 10 km?}
  B -- No --> X[No notification]
  B -- Yes --> C{Authenticated foreground socket?}
  C -- Yes --> D[Emit private Socket.IO event\nshow in-app toast]
  C -- No --> E[Send FCM data message]
  E --> F[Service worker shows browser notification]
  D --> G[Consumer opens Feed / My Claims]
  F --> G
```
...
```mermaid
flowchart LR
  V[Vercel\nReact static build] -->|HTTPS + Socket.IO| R[Render\nNode / Express API]
  R --> A[MongoDB Atlas]
  R --> C[Cloudinary]
  R --> F[Firebase Admin]
...
| Name | GitHub |
|------|--------|
| Your Name | [Sivag28](https://github.com/Sivag28) |
| Member 2 | [@member2](https://github.com/member2) |
| Member 3 | [@member3](https://github.com/member3) |
