...
const ConsumerFeed = () => {
  ...
  const filtered = useMemo(() => listings.filter((item) => !expiredListingIds.has(item._id)
    && (foodTypeFilter === 'all' || item.foodType === foodTypeFilter)
    && [item.foodName, item.description, item.category, item.merchantId?.shopName, item.merchantId?.city].filter(Boolean).some((value) => value.toLowerCase().includes(search.toLowerCase()))), [expiredListingIds, foodTypeFilter, listings, search]);
  const leave = async () => {
    if (await logout()) navigate('/login');
  };

  return <div className="app-shell find-food-shell min-h-screen bg-stone-50">
    {claimingId && <ProcessingIndicator message="🍽️ Processing claim..." />}
    ...
    <main className="mx-auto max-w-[1600px] p-4 pb-10 md:p-6 md:pb-12">
      <section className="relative mb-7 overflow-hidden rounded-3xl border border-amber-950/70 bg-gradient-to-br from-[#a15624] via-[#f67f3a] to-[#a15624] px-6 py-8 text-white shadow-xl shadow-amber-950/20 md:px-10 md:py-10"><div className="absolute -right-12 -top-20 h-64 w-64 rounded-full bg-amber-400/25 blur-2xl" /><div className="absolute bottom-0 right-1/4 h-32 w-32 rounded-full bg-orange-400/25 blur-xl" /><div aria-hidden="true" className="pointer-events-none absolute inset-y-0 right-0 hidden w-[42%] lg:block"><span className="absolute right-[28%] top-[18%] animate-[bounce_6s_ease-in-out_infinite] text-5xl opacity-90 drop-shadow-xl">🍛</span><span className="absolute right-[8%] top-[18%] animate-[bounce_7s_ease-in-out_infinite_0.8s] text-4xl opacity-85 drop-shadow-xl">🍕</span><span className="absolute right-[43%] top-[48%] animate-[bounce_5s_ease-in-out_infinite_0.4s] text-4xl opacity-85 drop-shadow-xl">🥐</span><span className="absolute right-[24%] top-[57%] animate-[bounce_7s_ease-in-out_infinite_1.2s] text-5xl opacity-90 drop-shadow-xl">🥗</span><span className="absolute right-[2%] bottom-[9%] animate-[bounce_6s_ease-in-out_infinite_0.6s] text-4xl opacity-85 drop-shadow-xl">🍱</span><span className="absolute right-[48%] bottom-[12%] animate-[bounce_5.5s_ease-in-out_infinite_1s] text-3xl opacity-80">✨</span></div><div className="relative max-w-2xl"><div className="mb-3 flex w-fit items-center gap-2 rounded-full border border-amber-100/20 bg-amber-50/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-amber-500 font-extrabold"><FaLeaf /> Less waste, more good</div><h2 className="text-3xl font-extrabold leading-tight tracking-tight md:text-4xl text-stone-900">A great meal is waiting nearby.</h2><p className="mt-3 max-w-lg text-sm leading-1 text-amber-50/80 md:text-base text-amber-200 font-extrabold">Discover fresh surplus food from local shops and give every good meal a second chance.</p><div className="mt-6 flex flex-wrap gap-3 text-sm"><span className="rounded-full bg-amber-50/10 px-3 py-2 font-extrabold text-amber-900"><FaMapMarkerAlt className="mr-2 inline text-amber-900" />Within 10 km</span><span className="rounded-full bg-amber-50/10 px-3 py-2 font-extrabold text-amber-900"><FaUtensils className="mr-2 inline text-amber-900" />{listings.length} offers today</span></div></div></section>
      <section className="mb-8"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-700">Choose your preference</p><h3 className="mt-1 text-xl font-extrabold text-slate-900">What are you in the mood for?</h3></div>{foodTypeFilter !== 'all' && <button onClick={() => setFoodTypeFilter('all')} className="text-sm font-semibold text-amber-700 hover:text-amber-800">Show all food</button>}</div><div className="grid gap-4 sm:grid-cols-2"><button onClick={() => setFoodTypeFilter('veg')} className={`group flex min-h-36 items-center gap-5 rounded-3xl border p-6 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${foodTypeFilter === 'veg' ? 'border-emerald-600 bg-emerald-600 text-white shadow-emerald-200' : 'border-emerald-200 bg-gradient-to-br from-emerald-50 to-lime-50 text-emerald-950'}`}><span className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl text-3xl ${foodTypeFilter === 'veg' ? 'bg-white/20 text-white' : 'bg-emerald-600 text-white'}`}><FaLeaf /></span><span><span className="block text-2xl font-extrabold">Veg</span><span className={`mt-1 block text-sm ${foodTypeFilter === 'veg' ? 'text-emerald-50' : 'text-emerald-800'}`}>Explore vegetarian food offers</span></span></button><button onClick={() => setFoodTypeFilter('non-veg')} className={`group flex min-h-36 items-center gap-5 rounded-3xl border p-6 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${foodTypeFilter === 'non-veg' ? 'border-rose-600 bg-rose-600 text-white shadow-rose-200' : 'border-rose-200 bg-gradient-to-br from-rose-50 to-orange-50 text-rose-950'}`}><span className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl text-3xl ${foodTypeFilter === 'non-veg' ? 'bg-white/20 text-white' : 'bg-rose-600 text-white'}`}><FaDrumstickBite /></span><span><span className="block text-2xl font-extrabold">Non-veg</span><span className={`mt-1 block text-sm ${foodTypeFilter === 'non-veg' ? 'text-rose-50' : 'text-rose-800'}`}>Explore non-vegetarian food offers</span></span></button></div></section>
      <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-700">Discover offers</p><h3 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-900 md:text-3xl">{foodTypeFilter === 'veg' ? 'Vegetarian food near you' : foodTypeFilter === 'non-veg' ? 'Non-vegetarian food near you' : 'Fresh food near you'}</h3><p className="mt-1 text-slate-500">Claim end-of-day offers before they are gone.</p></div><label className="relative block w-full md:max-w-md"><FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-600" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search food, shop, or category" className="w-full rounded-2xl border border-amber-900/25 bg-white py-3.5 pl-11 pr-4 text-sm shadow-sm outline-none focus:border-amber-500" /></label></div>
      {locationStatus === 'ready' && trending.length > 0 && <section className="mb-8 rounded-2xl border border-amber-900 bg-gradient-to-r from-orange-50 to-amber-50 p-5"><div className="flex items-center gap-2"><FaFire className="text-xl text-orange-500" /><div><p className="text-xs font-bold uppercase tracking-wider text-orange-600">Trending items</p><h3 className="text-xl font-bold text-slate-900">Trending now</h3></div></div><div className="mt-4 grid gap-3 md:grid-cols-3">{trending.map((item) => <article key={item._id} className="rounded-xl border border-amber-900/20 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h4 className="font-bold text-slate-900">{item.foodName}</h4><p className="mt-1 flex items-center gap-1.5 text-sm text-slate-600"><FaStore className="text-amber-600" />{item.shopName || 'Local store'}</p><p className="mt-1 flex items-start gap-1.5 text-sm text-slate-500"><FaMapMarkerAlt className="mt-1 shrink-0 text-amber-600" />{item.shopAddress || item.city || 'Nearby'}</p></div><span className="shrink-0 rounded-full bg-orange-100 px-2.5 py-1 text-xs font-semibold text-orange-700">{item.claimCount} {item.claimCount === 1 ? 'claim' : 'claims'}</span></div></article>)}</div><p className="mt-4 text-xs text-slate-500">Based on claims made in the last 10 minutes.</p></section>}
      {loading ? <div className="rounded-3xl border border-amber-900/25 bg-white p-12 text-center text-slate-500">Finding nearby offers…</div> : locationStatus !== 'ready' ? <div className="rounded-3xl border border-dashed border-amber-900/30 bg-white p-12 text-center"><FaMapMarkerAlt className="mx-auto text-4xl text-amber-400" /><h3 className="mt-4 text-lg font-bold">Location is needed</h3><p className="mt-1 text-slate-500">Set a valid location in your profile, or allow browser location access, to see nearby food offers.</p></div> : filtered.length === 0 ? <div className="rounded-3xl border border-dashed border-amber-900/30 bg-white p-12 text-center"><FaBoxOpen className="mx-auto text-4xl text-amber-300" /><h3 className="mt-4 text-lg font-bold">No nearby offers right now</h3><p className="mt-1 text-slate-500">There are no active offers within 10 km. Try again soon.</p></div> : <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((item) => { const discountPercentage = getDiscountPercentage(item.originalPrice, item.discountedPrice); return <article key={item._id} className="group overflow-hidden rounded-3xl border border-amber-900/25 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/10"><div className="relative h-52 overflow-hidden bg-amber-50"><span className="absolute left-4 top-4 z-10 rounded-full bg-white/90 px-3 py-1.5 text-xs font-bold capitalize text-slate-700 shadow-sm backdrop-blur">{item.category || 'Food offer'}</span>{discountPercentage && <span className="absolute right-0 top-4 z-20 rounded-l-2xl border-y border-l border-blue-900/20 bg-blue-600 px-4 py-2 text-center text-white shadow-lg shadow-red-950/30"><strong className="block text-2xl font-black leading-none">{discountPercentage}</strong><small className="mt-0.5 block text-[10px] font-extrabold uppercase tracking-[0.16em]">Off</small></span>}<span className="absolute bottom-4 right-4 z-10 rounded-full bg-emerald-500 px-3 py-1.5 text-xs font-bold text-white shadow-sm">{item.quantity} left</span>{imageUrl(item) ? <img src={imageUrl(item)} alt={item.foodName} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center bg-gradient-to-br from-amber-100 to-orange-50 text-5xl text-amber-400"><FaUtensils /></div>}</div><div className="p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl font-extrabold tracking-tight text-slate-900">{item.foodName}</h3><p className="mt-1 flex items-center gap-1.5 text-sm font-medium text-slate-500"><FaStore className="text-amber-600" />{item.merchantId?.shopName || 'Local store'}</p></div><div className="text-right"><p className="text-lg font-extrabold text-amber-700">₹{item.discountedPrice}</p><p className="text-xs text-slate-400 line-through">₹{item.originalPrice}</p></div></div><p className="mt-3 text-sm leading-5 text-slate-600">{item.description || 'Fresh surplus food available for pickup.'}</p><div className="mt-4 space-y-2 border-t border-slate-100 pt-4 text-sm text-slate-500"><p className="flex items-start gap-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-50 text-amber-600"><FaMapMarkerAlt /></span><span>{item.merchantId?.shopAddress || item.merchantId?.city || 'Nearby'}</span></p><p className="flex items-center gap-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-50 text-amber-600"><FaClock /></span>Pickup {formatIndianTime(item.pickupStart)} – {formatIndianTime(item.pickupEnd)} IST</p></div><PickupWindowCountdown listing={item} onExpired={() => removeExpiredListing(item._id)} /><button disabled={claimingId === item._id} onClick={() => claimFood(item)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-amber-600 py-3 font-bold text-white hover:bg-amber-700 disabled:opacity-60">{claimingId === item._id ? 'Claiming…' : <>Claim food <FaArrowRight /></>}</button></div></article>; })}</div>}
    </main></div>;
};
